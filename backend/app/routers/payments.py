...
@router.get("/pricing")
def get_pricing():
    return {"moneda": MONEDA_DEFECTO, "premium": {"precio": PRECIO_SUSCRIPCION_PREMIUM, "dias": SUSCRIPCION_PREMIUM_DIAS}, "tarea_destacada": {"precio": PRECIO_TAREA_DESTACADA, "dias": TAREA_DESTACADA_DIAS}, "anuncio": {"precio_por_dia": PRECIO_ANUNCIO_POR_DIA}}
...
def request_sponsor_ad(body: SponsorAdRequest, db=Depends(get_db), current_user=Depends(get_current_user)):
    ...
    detalle = {"marca": body.marca, "texto": body.texto, "url_destino": body.url_destino, "contacto": body.contacto, "categoria_id": body.categoria_id, "dias": body.dias}
    payment = Payment(user_id=current_user.id, tipo=PaymentType.ANUNCIO, monto=monto, moneda=MONEDA_DEFECTO, notas=json.dumps(detalle))
    db.add(payment); db.commit(); db.refresh(payment); return payment
...
def confirm_payment(payment_id, db=Depends(get_db), admin=Depends(get_current_admin)):
    ...
    try:
        if payment.tipo == PaymentType.SUSCRIPCION_TRABAJADOR:
            ...
            db.add(Ad(marca=detalle.get("marca", "—"), texto=detalle.get("texto", ""), url_destino=detalle.get("url_destino"), contacto=detalle.get("contacto"), categoria_id=detalle.get("categoria_id"), activo=True, fecha_inicio=now, fecha_fin=expiry, payment_id=payment.id))
