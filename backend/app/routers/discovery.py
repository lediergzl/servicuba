...
def _public_map_items(items: list[dict]) -> list[dict]:
    ...
    for source, target in zip(items, safe):
        if source.get("lat") is not None and source.get("lng") is not None:
            target["lat"] = round(float(source["lat"]), 2); target["lng"] = round(float(source["lng"]), 2)
    return safe
...
def _slug(value: str) -> str:
    return re.sub(r"[^a-z0-9]+", "-", value.lower().strip().replace("ñ", "n")).strip("-")
...
def discover_tasks(lat: Optional[float]=Query(None), lng: Optional[float]=Query(None), radius_km: float=Query(3.0,ge=0.1,le=10), category_id: Optional[int]=None, db: Session=Depends(get_db)):
    if not _valid_coordinates(lat,lng): return []
...
def directory_municipios(db: Session=Depends(get_db)):
    rows=(db.query(User.municipio).filter(User.es_trabajador.is_(True),User.suspendido.is_(False),User.municipio.isnot(None),func.length(func.trim(User.municipio))>=2).distinct().all())
    return sorted({r[0].strip() for r in rows if r[0] and r[0].strip()},key=str.casefold)
...
def discover_directory(municipio: Optional[str]=Query(None,max_length=100), tipo: str=Query("oferta",pattern="^(oferta|necesidad)$"), category_id: Optional[int]=None, db: Session=Depends(get_db)):
    ...
    clean=municipio.strip()
    q=(db.query(User,Category.nombre.label("categoria_nombre"),Category.icono.label("categoria_icono")).outerjoin(Category,Category.id==User.categoria_id).filter(User.es_trabajador.is_(True),User.suspendido.is_(False),User.municipio.isnot(None),func.lower(func.trim(User.municipio))==func.lower(clean)))
    ...
    return [{"id":str(u.id),"nombre":u.nombre,"foto":u.foto,"categoria_id":u.categoria_id,"categoria_nombre":cat,"categoria_icono":icon,"descripcion_trabajador":u.descripcion_trabajador,"precio_hora":u.precio_hora,"rating":u.rating or 0,"verificado":bool(u.verificado),"municipio":u.municipio,"zona":u.zona,"lat":u.lat,"lng":u.lng} for u,cat,icon in rows]
...
def recent_activity(limit:int=Query(6,ge=1,le=12),db:Session=Depends(get_db)):
    ...
    return [{"id":str(t.id),"titulo":t.titulo,"tipo":t.tipo,"categoria_nombre":c,"categoria_icono":i,"municipio":t.municipio,"created_at":t.created_at.isoformat() if t.created_at else None} for t,c,i in rows]
...
def seo_service_page(service_slug:str,city_slug:str,db:Session=Depends(get_db)):
    ...
    count=db.query(User).filter(User.es_trabajador.is_(True),User.suspendido.is_(False),User.categoria_id==category.id,func.lower(func.trim(User.municipio))==func.lower(city_name)).count() if category else 0
    title=f"{service_name} en {city_name} | ServiCuba"; description=f"Encuentra servicios de {service_name.lower()} en {city_name}. ServiCuba conecta personas con trabajadores locales."
    return f'''<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title}</title><meta name="description" content="{description}"></head><body><main><h1>{service_name} en {city_name}</h1><p>{description}</p><p>Hay actualmente {count} trabajador(es) con perfil activo en este municipio.</p><p><a href="/">Volver a ServiCuba</a></p></main></body></html>'''
