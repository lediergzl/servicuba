...
def moderation_reports(
    ...
    } for report, task, reporter in rows]
...
def dismiss_report(report_id: str, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
    ...
    report.status = ReportStatus.DESCARTADA
    report.reviewed_by = admin.id
    report.reviewed_at = datetime.utcnow()
    _audit(db, admin, "REPORT_DISMISSED", "report", report_id)
    db.commit()
...
def hide_reported_task(report_id: str, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
    ...
    report.status = ReportStatus.ACCIONADA
    report.reviewed_by = admin.id
    report.reviewed_at = datetime.utcnow()
    _audit(db, admin, "TASK_HIDDEN_BY_MODERATION", "task", str(task.id), details=f"report:{report_id}")
    db.commit()
...
def restore_reported_task(report_id: str, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
    ...
    report.status = ReportStatus.REVISADA
    report.reviewed_by = admin.id
    report.reviewed_at = datetime.utcnow()
    _audit(db, admin, "TASK_RESTORED_BY_MODERATION", "task", str(task.id), details=f"report:{report_id}")
    db.commit()
    return {"ok": True, "task_id": str(task.id), "task_status": task.estado.value, "report_status": report.status.value}
...
def suspend_reported_user(report_id: str, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
    ...
    report.reviewed_by = admin.id
    report.reviewed_at = datetime.utcnow()
    _audit(db, admin, "USER_SUSPENDED_FROM_REPORT", "user", str(user.id), details=f"report:{report_id}")
    db.commit()
...
def payment_reconciliation(start: datetime | None = None, end: datetime | None = None, moneda: str | None = None, tipo: PaymentType | None = None, estado: PaymentStatus | None = None, db: Session = Depends(get_db), _admin: User = Depends(get_current_admin)):
    ...
    counts = {s.value: 0 for s in PaymentStatus}; by_currency = {}; by_type = {}
    ...
    for payment in rows:
        ...
        if status in (PaymentStatus.CONFIRMADO, PaymentStatus.REEMBOLSADO):
            cb["confirmado_bruto"] = f"{Decimal(cb['confirmado_bruto']) + amount:.2f}"; tb["confirmado_bruto"] = f"{Decimal(tb['confirmado_bruto']) + amount:.2f}"
        if status == PaymentStatus.REEMBOLSADO:
            cb["reembolsado"] = f"{Decimal(cb['reembolsado']) + amount:.2f}"; tb["reembolsado"] = f"{Decimal(tb['reembolsado']) + amount:.2f}"
        elif status == PaymentStatus.CONFIRMADO:
            cb["neto"] = f"{Decimal(cb['neto']) + amount:.2f}"; tb["neto"] = f"{Decimal(tb['neto']) + amount:.2f}"
        elif status == PaymentStatus.PENDIENTE:
            cb["pendiente"] = f"{Decimal(cb['pendiente']) + amount:.2f}"; tb["pendiente"] = f"{Decimal(tb['pendiente']) + amount:.2f}"
        elif status == PaymentStatus.RECHAZADO:
            cb["rechazado"] = f"{Decimal(cb['rechazado']) + amount:.2f}"; tb["rechazado"] = f"{Decimal(tb['rechazado']) + amount:.2f}"
