...
async def sensitive_endpoint_rate_limit(request: Request, call_next):
    if request.method in {"POST", "PUT", "PATCH", "DELETE"}:
        ...
        matched = next(
            (p for p in _RATE_WINDOWS if path == p or path.startswith(p + "/")),
            None,
        )
        if matched:
...
# Database schema changes are managed by Alembic before the web process starts.
# Keep application startup free of ALTER TABLE/CREATE INDEX operations.
...
app.include_router(native_push.router, prefix="/api/push", tags=["Native Push"])
...
def public_sitemap():
    ...
    for city in SEO_CITIES:
        for service in SEO_SERVICES:
            urls.append(f"https://servicuba.onrender.com/servicios/{service}/{city}")
    xml = (
        '<?xml version="1.0" encoding="UTF-8"?><urlset '
        'xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'
        + "".join(
            f'<url><loc>{u}</loc><changefreq>daily</changefreq>'
            f'<priority>{"1.0" if u.endswith("/") else "0.7"}</priority></url>'
            for u in urls
        )
        + "</urlset>"
    )
