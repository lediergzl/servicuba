...
async function runNavigation(action) {
    ...
    try { return await action(); } finally { setNavigationBusy(false); }
}
...
function installHeaderResponsiveNav() {
    ...
    const accountGuest = document.getElementById('user-menu-guest');
    const accountAuth = document.getElementById('user-menu-auth');
    const account = accountGuest || accountAuth;
    if (account) account.classList.add('app-header__account');
    if (accountAuth) accountAuth.classList.add('app-header__account-auth');
    if (accountGuest) accountGuest.classList.add('app-header__account-guest');
...
async function renderWorkerActivationForm(profile = {}) {
    ...
    if (!container || profile.es_trabajador) { if (container) container.innerHTML = ''; return; }
    ...
    try {
        ...
        categories.forEach(cat => { const option = document.createElement('option'); option.value = cat.id; option.textContent = `${cat.icono || ''} ${cat.nombre}`.trim(); if (String(cat.id) === String(profile.categoria_id || '')) option.selected = true; select.appendChild(option); });
    ...
    document.getElementById('workerGpsBtn')?.addEventListener('click', async () => {
        const btn = document.getElementById('workerGpsBtn'); if (!btn || btn.disabled) return;
        const original = btn.textContent; btn.disabled = true; btn.textContent = 'Obteniendo ubicación…';
        try { const pos = await getGeolocation(); lat = Number(pos.coords.latitude); lng = Number(pos.coords.longitude); if (!Number.isFinite(lat) || !Number.isFinite(lng)) throw new Error('Coordenadas inválidas'); btn.textContent = '✓ Ubicación obtenida'; notify('Ubicación GPS obtenida correctamente.', 'success'); }
    ...
    document.getElementById('workerActivationForm')?.addEventListener('submit', async e => {
        e.preventDefault(); if (navigationBusy) return; const form = e.currentTarget; const submit = form.querySelector('button[type="submit"]'); const categoria_id = Number(select.value); if (!categoria_id) return notify('Selecciona tu oficio.', 'error');
        submit.disabled = true; submit.textContent = 'Activando perfil…';
        try {
            const updated = await apiFetch('/users/activar-trabajador', { method: 'PUT', body: JSON.stringify({ categoria_id, descripcion_trabajador: document.getElementById('workerDescripcion').value.trim() || null, precio_hora: document.getElementById('workerPrecioHora').value ? Number(document.getElementById('workerPrecioHora').value) : null, municipio: document.getElementById('workerMunicipio').value.trim() || null, zona: document.getElementById('workerZona').value.trim() || null, lat, lng }) });
            syncModeSwitch('trabajador'); setCurrentUserId(updated.id || updated.user_id); notify('Perfil de trabajador activado correctamente.', 'success'); showDashboardTrabajador();
        } catch (err) { notify(`No se pudo activar el perfil: ${err.message}`, 'error'); }
        finally { submit.disabled = false; submit.textContent = 'Activar perfil de trabajador'; }
    });
}
...
async function openWorkerView() {
    ...
    try {
        ...
        if (!me.es_trabajador) { syncModeSwitch('trabajador'); switchView('perfilView'); await loadProfileView(); notify('Completa tu perfil profesional aquí para activar el modo Trabajador.', 'info'); return; }
        setCurrentUserId(me.id || me.user_id); setAuthUi(); syncModeSwitch('trabajador'); showDashboardTrabajador();
...
async function openMessagesView() {
    ...
    try {
        ...
        list.querySelectorAll('[data-chat-task]').forEach(btn => btn.addEventListener('click', () => { if (!btn.disabled) { btn.disabled = true; Promise.resolve(openChatForTask(btn.dataset.chatTask)).finally(() => { btn.disabled = false; }); } }));
...
function wireGlobalButtons() {
    ...
    document.querySelectorAll('[data-view]').forEach(btn => btn.addEventListener('click', () => runNavigation(async () => {
    ...
    document.querySelectorAll('[data-modo]').forEach(btn => btn.addEventListener('click', () => runNavigation(async () => {
        const modo = btn.dataset.modo;
        if (modo === 'trabajador') return openWorkerView();
        if (modo === 'cliente') {
            if (!localStorage.getItem('token')) return showLogin();
            try { await apiFetch('/users/modo-activo', { method: 'PUT', body: JSON.stringify({ modo: 'cliente' }) }); syncModeSwitch('cliente'); showDashboardCliente(); }
            catch (err) { notify(`No se pudo cambiar al modo Cliente: ${err.message}`, 'error'); }
        }
    })));
}
...
async function restoreSession() {
    ...
    try {
        const me = await apiFetch('/auth/me'); setCurrentUserId(me.id || me.user_id); setAuthUi(); setModeFromUser(me);
