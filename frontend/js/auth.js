...
export function initAuth() {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                // Login es una operación pública. Un 401 aquí significa credenciales inválidas,
                // no sesión expirada; por eso no pasa por apiFetch().
...
function initForgotPassword() {
    document.getElementById('forgotPasswordBtn')?.addEventListener('click', async () => {
        ...
        let resp;
        try { resp = await apiFetch('/auth/forgot-password', { method: 'POST', body: JSON.stringify({ telefono: normalizePhone(step1.telefono) }) }); }
