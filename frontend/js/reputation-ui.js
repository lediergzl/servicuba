...
// La reputación es enriquecimiento: si falla, el perfil sigue funcionando.
...
async function renderForUser(user) {
    ...
    try {
        ...
        if (!total) { card.innerHTML='<div class="sc-reputation__head"><h3 class="sc-reputation__title">Reputación</h3></div><p class="sc-reputation__empty">Todavía no hay trabajos valorados. Cuando completes tu primer servicio confirmado, podrás convertir esa experiencia en una referencia para futuros clientes.</p>'; return; }
...
function init() {
    ...
    const host=document.getElementById('perfilContenido');
    if(host){ const observer=new MutationObserver(()=>{ if(host.children.length) setTimeout(refreshProfileReputation,0); }); observer.observe(host,{childList:true}); }
}
