...
/**
 * Wrapper de fetch que agrega el token, maneja 401 de forma centralizada
 * y lanza errores legibles con el mensaje del backend.
 */
...
// ---------- UI: toasts (reemplaza alert) y modales (reemplaza prompt) ----------
...
/** Notificación no bloqueante. type: 'info' | 'success' | 'error' */
...
export function showFormModal({ title, fields, confirmLabel = 'Guardar', cancelLabel = 'Cancelar' }) {
    ...
    return new Promise((resolve) => {
        ...
        const firstInput = Object.values(inputs)[0]?.input;
        firstInput?.focus();
...
/** Confirmación simple sí/no basada en el mismo sistema de modales. */
...
export function geolocationErrorMessage(err) {
    switch (err.code) {
        case err.PERMISSION_DENIED:
            return 'Activa el permiso de ubicación para continuar.';
        case err.POSITION_UNAVAILABLE:
            return 'No se pudo determinar tu ubicación. Verifica el GPS.';
        case err.TIMEOUT:
