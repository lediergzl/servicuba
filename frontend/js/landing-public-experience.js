...
function injectStyles() {
    ...
    style.textContent = `
        ...
        @media (min-width:761px){body:has(#landing:not(.hidden)) .app-shell{max-width:none;width:100%;min-height:100vh}body:has(#landing:not(.hidden)) .app-main{padding:0}body:has(#landing:not(.hidden)) .app-header{width:100%;padding:15px max(28px,calc((100vw - 1160px)/2));gap:32px;justify-content:flex-start}body:has(#landing:not(.hidden)) .app-header__brand{flex:0 0 auto;gap:10px}body:has(#landing:not(.hidden)) .logo-mark{width:32px;height:32px}body:has(#landing:not(.hidden)) .app-header__wordmark{font-size:24px;letter-spacing:.015em}body:has(#landing:not(.hidden)) #user-menu-guest{margin-left:auto}body:has(#landing:not(.hidden)) #landing.landing-product{padding-top:72px;padding-bottom:70px}body:has(#landing:not(.hidden)) .landing-product__grid{gap:72px}body:has(#landing:not(.hidden)) .landing-product__activity{padding-left:34px}body:has(#landing:not(.hidden)) .landing-product__title{font-size:clamp(54px,6.4vw,82px);max-width:760px}body:has(#landing:not(.hidden)) .landing-product__mechanism{max-width:680px;font-size:19px}body:has(#landing:not(.hidden)) .landing-product__roles{margin-top:58px}}
    `;
...
function ensurePublicChrome(){
    // Nota: el header ya tiene su propia navegación global (#servicubaHeaderNav,
    // instalada por app.js) que cubre exactamente estos mismos 4 accesos.
    // Antes se insertaba aquí un segundo <nav class="public-header-nav"> con
    // los mismos botones, causando que el menú apareciera duplicado en el
    // header (visible en escritorio, cuando la landing está activa).
    // No se agrega nada más: la navegación de app.js ya es suficiente.
}
...
function wireProductDirectoryButton(){
    const btn=document.getElementById('browseByMunicipioBtn');
    if(!btn||btn.dataset.directoryWired==='1')return;
    btn.dataset.directoryWired='1';
